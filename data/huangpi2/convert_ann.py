...
DATASETDIR = os.path.join(HOMEDIR, 'data/huangpi2/')
ann_dirs = [x for x in os.listdir(DATASETDIR) if 'A' not in x and 'I' not in x]
print(ann_dirs)
if __name__ == "__main__":

    for ann_dir in ann_dirs:
        anno = json.load(open(os.path.join(DATASETDIR, ann_dir, 'multi_person.json'), 'r'))
        for obj in tqdm(anno):
            ...
            with open(os.path.join(DATASETDIR, 'Annotations', im_filename[:-4] + '.json'), 'w') as new_ann:
                try:
                    img = cv2.imread(os.path.join(DATASETDIR, ann_dir, im_filename))
                    height, width, _ = img.shape
                except:
                    print(os.path.join(DATASETDIR, ann_dir, im_filename))
                    sys.exit(1)
            ...
            shutil.copy(os.path.join(DATASETDIR, ann_dir, im_filename), os.path.join(DATASETDIR, 'ImageSet', im_filename))
    img_names = [x[:-4] for x in os.listdir(os.path.join(DATASETDIR, 'ImageSet'))]
    img_names.sort()
    with open('huangpi2.txt', 'w') as f:
        f.write("\n".join(img_names))
