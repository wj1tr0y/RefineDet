...
void MultiBoxLossLayer<Ftype, Btype>::Forward_cpu(const vector<Blob*>& bottom,
  ...
  if (bottom.size() >= 5) {
    // print("arm_conf_data assign")
  arm_conf_data = bottom[4]->cpu_data<Dtype>();
   //LOG(INFO)<< arm_conf_data << "arm_conf_in_multibox:";
  //@Checked 
  }
  if (bottom.size() >= 6) {
  arm_loc_data = bottom[5]->cpu_data<Dtype>();
  //LOG(INFO) << arm_loc_data<< "arm_loc_in_multibox:";
  //@Checked
  ...
  if (bottom.size() >= 6) {
    //@add function CasRegFindMatches 
    //LOG(INFO) << "CasRegFindMatches";
    ...
    //original Code
    //LOG(INFO) << "FindMatches,bottom size:" << bottom.size() ;
